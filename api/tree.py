...
def get_node_inputs(x):
    if IS_BLENDER_4:
        return [
            i
            for i in x.interface.items_tree
...
def tree(name):
    ...
    def build_tree(builder):
        ...
        input_count = sum(
            map(
                lambda p: len(p.annotation.__annotations__)
                if issubclass(p.annotation, InputGroup)
                else 1,
        ...
        def validate_param(param):
            if param.annotation == inspect.Parameter.empty:
                raise Exception(
                    f"Tree input '{param.name}' has no type specified. Please annotate with a valid node input type."
                )
            if not issubclass(param.annotation, Type):
                raise Exception(
                    f"Type of tree input '{param.name}' is not a valid 'Type' subclass."
                )
        ...
        for param in signature.parameters.values():
            if issubclass(param.annotation, InputGroup):
                ...
                prefix = (
                    (param.annotation.prefix + "_")
                    if hasattr(param.annotation, "prefix")
                    else ""
                )
                for group_param, annotation in param.annotation.__annotations__.items():
                    default = getattr(instance, group_param, None)
                    inputs[prefix + group_param] = (
                        annotation,
                        inspect.Parameter.empty if default is None else default,
                        param.name,
                        prefix,
                    )
            else:
        ...
        for i, arg in enumerate(inputs.items()):
            ...
            if len(node_inputs) > i:
                ...
                if IS_BLENDER_4:
                    node_input = node_group.interface.new_socket(
                        socket_type=arg[1][0].socket_type,
                        name=input_name,
                        in_out="INPUT",
                    )
                else:
                    node_input = node_group.inputs.new(
                        arg[1][0].socket_type, input_name
                    )
            if arg[1][1] != inspect.Parameter.empty:
                node_input.default_value = arg[1][1]
            if arg[1][2] is not None:
                if arg[1][2] not in builder_inputs:
                    builder_inputs[arg[1][2]] = signature.parameters[
                        arg[1][2]
                    ].annotation()
                setattr(
                    builder_inputs[arg[1][2]],
                    arg[0].replace(arg[1][3], ""),
                    arg[1][0](group_input_node.outputs[i]),
                )
            else:
        ...
        if inspect.isgeneratorfunction(builder):
            ...
            if all(
                map(
                    lambda x: issubclass(type(x), Type)
                    and x._socket.type == "GEOMETRY",
                    generated_outputs,
                )
            ):
        ...
        if isinstance(outputs, dict):
            ...
            for i, (k, v) in enumerate(outputs.items()):
                ...
                if IS_BLENDER_4:
                    node_group.interface.new_socket(
                        socket_type=v.socket_type, name=k, in_out="OUTPUT"
                    )
                else:
            ...
            for i, result in enumerate(_as_iterable(outputs)):
                ...
                if IS_BLENDER_4:
                    node_group.interface.new_socket(
                        socket_type=result.socket_type, name="Result", in_out="OUTPUT"
                    )
                else:
